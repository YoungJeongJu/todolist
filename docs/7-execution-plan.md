...
| FE-08     | 캘린더 뷰 (월/주 보기)                                              | 프론트엔드  | 2~3시간       | FE-05, FE-06       |
| FE-07     | 통합 테스트 및 반응형 UI 검증                       | 프론트엔드  | 2~3시간       | FE-03~FE-08        |
...
**완료 조건**
- [x] `frontend/src/api/client.ts`에 `VITE_API_BASE_URL` 환경변수를 기본 URL로 사용하는 axios 인스턴스가 생성되어 있다
- [x] 인터셉터에서 JWT 토큰을 `Authorization: Bearer <token>` 헤더에 자동 첨부한다
- [x] 401 응답 수신 시 토큰을 삭제하고 `/login`으로 리다이렉트하는 인터셉터가 적용되어 있다
- [x] `frontend/src/api/authApi.ts`, `todoApi.ts`, `categoryApi.ts`에 각 도메인별 API 호출 함수가 구현되어 있다
- [x] `frontend/src/hooks/useAuth.ts`가 인증 상태(로그인 여부, 사용자 정보) 및 로그인·로그아웃 함수를 제공한다
- [x] `frontend/src/hooks/useTodos.ts`가 TanStack Query를 사용하여 할 일 목록을 조회하고, `isOverdue` 파생 필드를 계산하는 상태이다
- [x] `frontend/src/hooks/useCategories.ts`가 TanStack Query를 사용하여 카테고리 목록을 조회하는 상태이다
...
**완료 조건**
- [x] `frontend/src/pages/LoginPage.tsx`가 이메일·비밀번호 입력 폼을 렌더링하며, 로그인 성공 시 메인 화면(`/`)으로 이동한다 (UC-02, US-02)
- [x] 로그인 실패 시 API 오류 메시지를 화면에 표시한다 (US-22)
- [x] `frontend/src/pages/RegisterPage.tsx`가 이메일·비밀번호·이름 입력 폼을 렌더링하며, 가입 성공 시 메인 화면으로 이동한다 (UC-01, US-01)
- [x] 이메일 형식 오류, 비밀번호 규칙 오류, 이름 길이 오류를 폼 필드 단위로 표시한다 (US-18)
- [x] `frontend/src/pages/ProfilePage.tsx`가 이름 변경 폼과 비밀번호 변경 폼을 렌더링한다 (UC-03, US-03)
- [x] 비밀번호 변경 폼에서 현재 비밀번호 입력 필드가 포함되어 있다 (UC-03 AC-02)
- [x] 회원 탈퇴 버튼 클릭 시 확인 다이얼로그가 표시되고, 확인 후 탈퇴 API를 호출한다 (UC-04, US-04)
- [x] 탈퇴 성공 후 로그인 화면(`/login`)으로 이동한다 (UC-04 AC-03)
...
**완료 조건**
- [x] `frontend/src/components/category/CategoryList.tsx`가 로그인 사용자의 카테고리 목록을 렌더링한다 (UC-05 AC-02)
- [x] `frontend/src/components/category/CategoryForm.tsx`가 카테고리 이름 입력 필드와 생성 버튼을 포함한다 (UC-05, US-05)
- [x] 이름이 0자이거나 30자를 초과하는 경우 오류 메시지를 표시하고 API 요청을 차단한다 (UC-05 AC-01)
- [x] 카테고리 삭제 버튼 클릭 시 확인 절차가 제공된다 (UC-06 AC-03, US-06)
- [x] `is_default=true`인 카테고리의 삭제 버튼은 비활성화되거나 숨겨진 상태이다 (BR-06, US-21)
- [x] 삭제 성공 후 카테고리 목록이 즉시 갱신된다 (TanStack Query invalidation)
- [x] `useCategories` Hook을 통해 카테고리 데이터를 조회한다
...
**완료 조건**
- [x] `frontend/src/pages/MainPage.tsx`가 할 일 목록과 필터 영역을 포함하여 렌더링된다
- [x] `frontend/src/components/todo/TodoList.tsx`가 할 일 목록을 렌더링한다
- [x] `frontend/src/components/todo/TodoItem.tsx`가 제목, 상태, 기한, 카테고리를 표시한다
- [x] 기한 초과(`isOverdue=true`) 할 일이 시각적으로 구분되어 표시된다 (UC-11 AC-04, US-11)
- [x] 상태 필터(`미시작`, `진행 중`, `완료`, `기한 초과`) UI가 구현되어 있고, 선택 시 `useTodos` Hook에 필터 파라미터가 전달된다 (UC-11, US-11)
- [x] 카테고리 필터 UI가 구현되어 있고, 카테고리 선택 시 `categoryId` 파라미터가 API에 전달된다 (UC-12, US-12)
- [x] 상태 필터와 카테고리 필터를 동시에 적용할 수 있다 (UC-12 AC-02, US-17)
- [x] 필터 조건에 해당하는 할 일이 없을 때 빈 목록 안내 메시지를 표시한다 (US-11 예외 흐름)
- [x] `useTodos` Hook에서 OVERDUE 파생 상태가 계산되며, 컴포넌트에서 직접 계산하지 않는다
...
**완료 조건**
- [x] `frontend/src/components/todo/TodoForm.tsx`가 제목, 설명, 시작일, 종료일(캘린더 UI), 카테고리 선택 필드를 포함한다 (UC-07 AC-01~05, US-07)
- [x] 제목이 빈 값이거나 100자를 초과하면 오류 메시지를 표시하고 제출을 차단한다
- [x] 시작일이 비어있으면 오류 메시지를 표시하고 제출을 차단한다 (UC-07 AC-03)
- [x] 종료일이 비어있으면 오류 메시지를 표시하고 제출을 차단한다 (UC-07 AC-03)
- [x] `dueDate < startDate`인 경우 오류 메시지를 표시하고 제출을 차단한다 (BR-09, US-19)
- [x] 카테고리를 선택하지 않으면 자동으로 기본 카테고리가 적용된다 (BR-08, UC-07 AC-05)
- [x] 등록 성공 후 할 일 목록이 즉시 갱신된다 (TanStack Query invalidation)
- [x] 수정 모드에서 기존 값이 폼 필드에 미리 채워진 상태로 렌더링된다 (UC-08, US-08)
- [x] 삭제 버튼 클릭 시 확인 절차가 제공되고, 확인 후 삭제 API를 호출한다 (UC-09, US-09)
- [x] `TodoItem.tsx` 또는 별도 UI에서 상태 선택(드롭다운 또는 버튼)으로 `PATCH /api/todos/:id/status`를 호출한다 (UC-10, US-10)
- [x] 할 일 카드 클릭 시 수정 모달이 열린다 (연필 버튼 제거, 카드 전체가 클릭 영역)
...
**완료 조건**
- [x] US-15 (회원가입 → 카테고리 생성 → 할 일 등록) 전체 흐름이 오류 없이 완료된다
- [x] US-16 (기한 초과 필터링 → 상태 변경 또는 날짜 수정) 흐름이 정상 동작한다
- [x] US-17 (카테고리 삭제 → 이관 확인 → 신규 카테고리 생성 → 필터 재확인) 흐름이 정상 동작한다
- [x] US-18 (중복 이메일 가입 시도) 시 오류 메시지가 표시된다
- [x] US-19 (잘못된 날짜 등록 시도) 시 오류 메시지가 표시되고 저장이 차단된다
- [x] US-20 (미인증 상태에서 보호 라우트 접근) 시 `/login`으로 리다이렉트된다
- [x] US-21 (기본 카테고리 삭제 시도) 시 삭제가 차단된다
- [x] US-22 (비밀번호 불일치 로그인) 시 포괄적 오류 메시지가 표시된다
- [x] 모바일 뷰포트(375px 기준)에서 모든 주요 화면(로그인, 메인, 프로필)의 레이아웃이 깨지지 않는다
- [x] 캘린더 뷰에서 월 보기와 주 보기가 정상 전환된다 (UC-15 AC-02)
- [x] 캘린더 뷰에서 할 일 이벤트가 날짜에 맞게 표시되고, 상태별 색상이 적용된다 (UC-15 AC-03, AC-04)
- [x] 캘린더 뷰에서 이벤트 클릭 시 수정 모달이 열린다 (UC-15 AC-05)
- [x] 상태 필터와 카테고리 필터가 캘린더 뷰에도 적용된다 (UC-15 AC-06, AC-07)
